...
3. Run the script:
```bash
python SOC710_HS_extract_batch.py
```
...
## 概述
SOC710_HS_extract_batch.py是一个用于批量处理高光谱图像的Python工具，专门用于提取目标叶片区域并分析其光谱特征。该工具实现了从图像加载、植被指数计算到目标区域提取和结果可视化的完整工作流程，为研究人员提供高效的植物叶片高光谱数据分析能力。
